...
Dialers (`nng_dialer` objects) use the functions
`nng_dialer_get()` and
`nng_dialer_set()` to set and retrieve option
values.

Listeners (`nng_listener` objects) use the functions
`nng_listener_get()`
and `nng_listener_set()` to set and
