...
ws_send_close(nni_ws *ws, uint16_t code)
{
	...
	if (!nni_aio_schedule(aio, ws_cancel_close, ws)) {
		ws->wclose = false;
		ws_frame_fini(frame);
		return;
	}
...
ws_listener_accept(void *arg, nni_aio *aio)
{
	...
	if (nni_aio_schedule(aio, ws_accept_cancel, l)) {
...
ws_dialer_dial(void *arg, nni_aio *aio)
{
	...
	if (!nni_aio_schedule(aio, ws_dial_cancel, ws)) {
		nni_mtx_unlock(&d->mtx);
		ws_reap(ws);
		return;
	}
...
ws_str_send(void *arg, nni_aio *aio)
{
	...
	if (!nni_aio_schedule(aio, ws_write_cancel, ws)) {
		nni_mtx_unlock(&ws->mtx);
		ws_frame_fini(frame);
		return;
	}
...
ws_str_recv(void *arg, nng_aio *aio)
{
	...
	nni_mtx_lock(&ws->mtx);
	if (!nni_aio_schedule(aio, ws_read_cancel, ws)) {
		nni_mtx_unlock(&ws->mtx);
		return;
	}
