...
xreq0_pipe_init(void *arg, nni_pipe *pipe, void *s)
{
	...
	nni_aio_init(&p->aio_get, xreq0_get_cb, p);
	nni_aio_init(&p->aio_put, xreq0_put_cb, p);
...
// For raw mode we can just let the pipes "contend" via get to get a
...
xreq0_send_cb(void *arg)
{
	...
	nni_msgq_aio_get(p->req->uwq, &p->aio_get);
}

static void
xreq0_put_cb(void *arg)
{
	xreq0_pipe *p = arg;
...
xreq0_recv_cb(void *arg)
{
	...
	nni_msgq_aio_put(sock->urq, &p->aio_put);
}
